...
namespace OpenXROSX.Editor
{
    [InitializeOnLoad]
    internal static class OpenXRRuntimeAutoSelector
    {
        ...
        private const string EditorPrefKey = "OpenXROSX.OpenXR.CustomRuntimeJson";

        static OpenXRRuntimeAutoSelector()
        {
            EditorApplication.delayCall += ApplyConfiguredRuntime;
        }

        [MenuItem("Tools/OpenXR/Use OpenXR OSX Runtime")]
        private static void UseDefaultRuntime()
        {
            ...
            if (string.IsNullOrEmpty(runtimePath) || !File.Exists(runtimePath))
            {
                runtimePath = EditorUtility.OpenFilePanel(
                    "Select OpenXR OSX Runtime json",
            ...
            {
                Debug.LogWarning("No OpenXR runtime json selected. Build the runtime or set XR_RUNTIME_JSON first.");
                return;
            }
        ...
        private static void ChooseCustomRuntime()
        {
            ...
            string selectedPath = EditorUtility.OpenFilePanel("Select OpenXR Runtime json", directory, "json");
        ...
        private static void ClearForcedRuntime()
        {
            ...
            Debug.Log("OpenXR runtime override cleared for this Unity editor session.");
        }
        ...
        private static void LogActiveRuntime()
        {
            ...
            Debug.Log(
                $"Configured OpenXR runtime: {configuredPath}\n" +
        ...
        private static void ApplyConfiguredRuntime(bool verbose)
        {
            ...
            if (string.IsNullOrEmpty(runtimePath) || !File.Exists(runtimePath))
            {
                if (verbose)
                {
                    Debug.LogWarning("OpenXR runtime json not found. Choose a runtime json or set XR_RUNTIME_JSON.");
                }
            ...
            if (verbose)
            {
                Debug.Log($"OpenXR runtime forced to: {runtimePath}");
            }
        }
    }
}
