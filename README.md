...
- The Quest/Android client feeds real hand joints into the runtime, supports WiFi UDP and retrying USB ADB reverse TCP streaming, matches per-frame render poses for smoother headset reprojection, and exposes a first-pass `XR_FB_foveation` path when supported by the headset.
