...
The repository now ships a local [`mise.toml`](mise.toml) for portable tool pinning.
...
Use Conventional Commits for commit subjects, for example `feat(runtime): add client refresh-rate negotiation` or `ci: cache build artifacts for faster pr feedback`. `commitlint` enforces this on pull requests. Tagged releases and `CHANGELOG.md` are cut manually by the maintainer; there is no CI publishing path.
