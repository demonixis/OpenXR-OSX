...
This page covers the native visionOS viewer in `clients/visionos/`.
...
- UDP stream connection through the shared `OpenXRStreaming` package
...
When using the Xcode UI, open `clients/OpenXR Clients.xcworkspace` rather than opening `Vision Player.xcodeproj`
while another client project is already open. `Vision Player` and `OpenXR Simulator` both depend on
the local `OpenXRStreaming` package, and Xcode can report `Missing package product 'OpenXRStreaming'`
...
2. Build and run `Vision Player` on visionOS Simulator or device.
